...
export class EmojiLibrary {
   ...
   */
  getAllEmojisAndTags () {
    return this.#converter.codePointsToEmojis(this.#rawEmojiObjectsArray)
  }
  ...
  getEmojiObjectsByCategory (...categories) {
    ...
    return this.#converter.codePointsToEmojis(emojiObjectsRaw)
  }
  ...
  getEmojisOnlyFromCategory (...categories) {
    const emojis = []
    const emojisAndTags = this.getEmojiObjectsByCategory(...categories)
    for (let i = 0; i < emojisAndTags.length; i++) {
      emojis.push(emojisAndTags[i].emoji)
    }
    return emojis
   ...
   */
  getEmojiByTag (tag) {
    const emoji = this.#getEmojiFromTag(tag)
